...
if [ $# -eq 2 ]
  then
    if [ $1 == "arm/v6" ]
      then
        rm -rf out/linux32
        docker buildx build --no-cache --progress=plain --platform=linux/386 --build-arg TARGETARCH=386 --build-arg BUILD_TYPE=$2 --build-arg REPO=https://github.com/trethore/java-chromium-embedded-framework.git --build-arg REF=master --file docker/DockerfileLinuxARMPrebuild --output out .
    fi
    docker buildx build --no-cache --progress=plain --platform=linux/$1 --build-arg TARGETARCH=$1 --build-arg BUILD_TYPE=$2 --build-arg REPO=https://github.com/trethore/java-chromium-embedded-framework.git --build-arg REF=master --file docker/DockerfileLinux --output out .
else
    if [ $1 == "arm/v6" ]
      then
        rm -rf out/linux32
        docker buildx build --no-cache --progress=plain --platform=linux/386 --build-arg TARGETARCH=386 --build-arg BUILD_TYPE=$2 --build-arg REPO=$3 --build-arg REF=$4 --file docker/DockerfileLinuxARMPrebuild --output out .
    fi
    docker buildx build --no-cache --progress=plain --platform=linux/$1 --build-arg TARGETARCH=$1 --build-arg BUILD_TYPE=$2 --build-arg REPO=$3 --build-arg REF=$4 --file docker/DockerfileLinux --output out .
fi
